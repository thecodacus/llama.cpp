...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor * dst) {
    ...
    // TurboQuant KV: only the vector kernel has turbo instances (there is no turbo
    // MMA/tile path). Route every batch size there instead of falling through to a
    // kernel that cannot handle these types, which would disable FA entirely and
    // blow up the attention compute buffer.
    {
